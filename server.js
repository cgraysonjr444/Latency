...
Deno.serve({ port: 10000 }, async (req) => {
  ...
  try {
    ...
    // 3. AUTH CALLBACK + GOOGLE FIT DATA FETCH
    if (path.includes("auth/callback")) {
      ...
      // Get Heart Rate from Google Fit (Last 5 Mins)
      let bpm = 72; 
      try {
      ...
      } catch (_e) { console.log("No recent Fit data found, using default."); }
    ...
    if (path === "/data") {
      ...
      } catch (_e) { return new Response("[]", { headers: { ...headers, "Content-Type": "application/json" } }); }
    }
